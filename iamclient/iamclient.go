...
func New(config *config.Config, cryptcoxontext *cryptutils.CryptoContext, insecure bool) (client *Client, err error) {
	...
	var secureProtectedOpt grpc.DialOption

	if insecure {
		secureProtectedOpt = grpc.WithInsecure()

		if client.publicConnection, err = grpc.DialContext(ctx, config.IAMPublicServerURL, grpc.WithInsecure(), grpc.WithBlock()); err != nil {
		...
		if tlsConfig, err = cryptcoxontext.GetClientMutualTLSConfig(certURL, keyURL); err != nil {
			return client, aoserrors.Wrap(err)
		}
